...
describe("HttpClient OAuth2 behavior", () => {
  ...
  test("api=true refreshes and sets Authorization header when token is expired", () => {
    const c = new HttpClient();
    c.oauth2Token = new OAuth2Token(
      "expired-token",
  ...
  test("api=false never sets Authorization header", () => {
    const c = new HttpClient();
    c.oauth2Token = new OAuth2Token("ok", Math.floor(Date.now() / 1000) + 3600);
